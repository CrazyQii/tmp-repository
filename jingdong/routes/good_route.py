...
def get_goods(analyze_id):
    ...
    try:
        ...
        create_goods(keyword_type=analyze['keyword_type'], keyword_brand=analyze['keyword_brand'])
        goods = db_util.batch_select(COLLECTION_GOOD_DETAIL, condition={'keyword_type': analyze['keyword_type'],
                                                                        'keyword_brand': analyze['keyword_brand']})
        for item in goods:
            item['_id'] = None
            result.append(item)
