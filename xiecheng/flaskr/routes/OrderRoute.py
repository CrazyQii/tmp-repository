...
def query_passenger():
    ...
    try:
        param = request_parse(request)
        if param.get('account_id') is not None and param.get('flight_id') is not None:
            passengers = PassengerModel.query.filter(PassengerModel.account_id == param.get('account_id')).filter(
                PassengerModel.flight_id == param.get('flight_id')).all()
...
def remove_passenger():
    ...
    try:
        param = request_parse(request)
        if param.get('account_id') is not None and param.get('flight_id') is not None and param.get(
                'id_card') is not None:
            db.session.query(PassengerModel).filter(PassengerModel.account_id == param.get('account_id')).filter(
                PassengerModel.flight_id == param.get('flight_id')).filter(
