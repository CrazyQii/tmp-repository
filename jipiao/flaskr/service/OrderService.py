...
class OrderService(object):
    ...
    @classmethod
    def query_orders(cls, account_id, order_by=OrderModel.pay_end_time):
        """
        分页多条件查询航班
        :param account_id:
        :param flight_id:
        ...
        if orders is not None:
            for item in orders:
                item = item.to_json()
                if item['pay_end_time'] < datetime.now() and item['pay_ready'] == 0:
                    item['pay_ready'] = 2
                    OrderModel.query.filter(OrderModel.id == item['id']).update(item)
